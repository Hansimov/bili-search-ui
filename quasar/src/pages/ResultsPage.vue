<template>
  ...
  >
    ...
    <ChatExportDialog
      ...
      :session-bundle="currentExportBundle"
      :initial-selected-rounds="exportDialogSelectedRounds"
    />
  </q-card>
</template>
...
import { buildPrefixExportRoundSelection } from 'src/services/chatExport';
...
export default {
  ...
  setup() {
    ...
    const isTranscriptLlmStreaming = computed(
      () =>
        (toolCall.value?.type === 'summarize_transcript' ||
          toolCall.value?.type === 'ask_transcript') &&
        toolCall.value?.status === 'streaming'
    );
    const showVideoResultsPanel = computed(
      () =>
        exploreStore.hasResults ||
        (exploreStore.isExploreLoading && !isTranscriptLlmStreaming.value)
    );
    ...
    const exportDialogSelectedRounds = ref([]);
    ...
    const openExportDialog = (payload = null) => {
      exportDialogSelectedRounds.value = Array.isArray(
        payload?.selectedRoundIndexes
      )
        ? [...payload.selectedRoundIndexes]
        : buildPrefixExportRoundSelection(
            currentExportBundle.value,
            payload?.maxRoundIndex
          );
